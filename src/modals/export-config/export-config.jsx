...
function ExportDialog({
    projectId, optimId, open, handleClose
}) {
    ...
    const {availableFrameworks, availableCodeSamples, codeSamples, config } = configState;
    useExportEffects(dispatch, projectId, optimId, frameworkTab, sampleType)

    useEffect(() => {
        const codeSamples = _.get(availableCodeSamples, availableFrameworks[frameworkTab], [])
        if (codeSamples.length > 0) {
            setSampleType(codeSamples[0])
        }
    }, [availableCodeSamples, availableFrameworks, frameworkTab]);

    const configExists = _.get(availableFrameworks, frameworkTab) && availableFrameworks[frameworkTab] in config;
    const codeExists = _.get(availableFrameworks, frameworkTab) && sampleType in _.get(codeSamples, `${availableFrameworks[frameworkTab]}`, {});
    return (
            ...
            <DialogContent>
                ...
                <FormControl variant="outlined"  className={classes.select}>
                    ...
                    <Select
                        ...
                        label="Code type"
                    >
                    {_.get(availableCodeSamples, availableFrameworks[frameworkTab], []).map(sampleTypeOption => (
                        <MenuItem value={sampleTypeOption} key={sampleTypeOption}>{sampleTypeOption}</MenuItem>
                    ))}
                    </Select>
                </FormControl>
                <CodeContainer
                    language="python"
                    text={codeExists ? _.get(codeSamples, `${availableFrameworks[frameworkTab]}.${sampleType}`) : ""}
                    defaultFileName="sample.py"
                />
            </DialogContent>
            <DialogActions>
                <Button onClick={() => handleClose()} color="secondary" className={classes.containedButton} variant="contained">
                    Close
                </Button>
            </DialogActions>
        </Dialog>
